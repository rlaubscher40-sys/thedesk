 ...
 */
export function initCrashLoopGuard(now: number = Date.now()): void {
  ...
  try {
    ...
    looping = isLooping;
    if (looping) {
      // Clear the breadcrumb trail so the recovery reload starts a fresh count
      // and we don't re-report on every subsequent crashed load.
      clearBoots();
    } else {
      writeBoots(boots);
    }
