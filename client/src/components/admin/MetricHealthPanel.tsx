...
export function MetricHealthPanel() {
  ...
  const attention = rows?.filter(
    (row) => row.state !== "within review window" || row.sourceStatus !== "link available"
  ).length;
  return (
    <section className="border-t border-[var(--color-border)] pt-6 space-y-3">
      ...
      <p className="text-sm">
        Reporting periods, successful storage and source links are tracked separately for each
        metric. A source link makes a figure traceable; it does not certify accuracy. Thresholds
        prompt review and do not predict the next release.
      </p>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr>
                  ...
                  <th className="p-2">Source link</th>
