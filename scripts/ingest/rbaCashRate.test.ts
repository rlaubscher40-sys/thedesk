...
const csv = readFileSync(
  new URL("./fixtures/rba-cash-daily.csv", import.meta.url),
  "utf8",
);
...
it("reads the daily target from current unquoted RBA CSV and preserves its observation date", () => {
  expect(parseCashRate(csv, now)).toEqual({
    rate: 4.35,
    asOf: new Date("2026-09-07"),
  });
  expect(
    parseCashRate(csv.replace("07-Sep-2026,4.35", '"07-Sep-2026","4.35"'), now),
  ).toEqual(parseCashRate(csv, now));
});
...
it("uses the Sydney day during daylight saving too", () => {
  ...
  expect(parseCashRate(summer, new Date("2026-01-07T13:30:00Z"))).toEqual({
    rate: 4.35,
    asOf: new Date("2026-01-07"),
  });
});

it.each([
  ["Sep", "2026-09-07T23:00:00Z", "2026-09-08"],
  ["Jan", "2026-01-07T13:30:00Z", "2026-01-08"],
])(
  "accepts a completed %s Sydney-day observation before UTC midnight",
  (month, checkedAt, asOf) => {
    const completed = csv
      .replace("08-Sep-2026,,", "08-Sep-2026,4.10,")
      .replaceAll("Sep", month);
    expect(parseCashRate(completed, new Date(checkedAt))).toEqual({
      rate: 4.1,
      asOf: new Date(asOf),
    });
  },
...
it("rejects duplicate completed latest observations instead of arbitrarily choosing one", () => {
  const completed = csv.replace("08-Sep-2026,,", "08-Sep-2026,4.10,");
  expect(() => parseCashRate(completed + "\n08-Sep-2026,4.35,\n", now)).toThrow(
    "duplicate",
  );
});

it("still rejects a pending row beyond its grace window and a genuinely future blank row", () => {
  expect(() => parseCashRate(csv, new Date("2026-09-13T01:00:00Z"))).toThrow(
    "unavailable",
  );
  expect(() =>
    parseCashRate(csv.replace("08-Sep-2026,,", "09-Sep-2026,,"), now),
  ).toThrow("unavailable");
});
...
it("permits one pending publication row across a long weekend and enforces the bound", () => {
  const friday = csv
    .replaceAll("08-Sep-2026", "11-Sep-2026")
    .replaceAll("07-Sep-2026", "10-Sep-2026");
  expect(parseCashRate(friday, new Date("2026-09-14T23:00:00Z"))).toEqual({
    rate: 4.35,
    asOf: new Date("2026-09-10"),
  });
  expect(() => parseCashRate(friday, new Date("2026-09-15T23:00:00Z"))).toThrow(
    "unavailable",
  );
});
...
it("never skips a second missing observation or a duplicate newest date", () => {
  const overnight = new Date("2026-09-08T15:38:57.888Z");
  expect(() =>
    parseCashRate(csv.replace("07-Sep-2026,4.35", "07-Sep-2026,"), overnight),
  ).toThrow("unavailable");
...
it("uses a newly populated latest target rather than retaining the previous rate", () => {
  expect(
    parseCashRate(
      csv.replace("08-Sep-2026,,", "08-Sep-2026,4.10,"),
      new Date("2026-09-08T15:38:57.888Z"),
    ),
  ).toEqual({
    rate: 4.1,
...
it("keeps the independent stale-observation check after allowing a pending publication row", () => {
  const old = csv.replace(/0[2-7]-Sep-2026/g, (date) =>
    date.replace("Sep", "Aug"),
  );
  expect(() =>
    parseCashRate(old, new Date("2026-09-08T15:38:57.888Z")),
  ).toThrow("Stale");
});
...
it("returns successful data without reporting a source error", async () => {
  ...
  const report = vi.fn();
  expect(await fetchCashRate(report)).toEqual({
    rate: 4.35,
    asOf: new Date("2026-09-07"),
  });
...
it("reports the HTTP status without including the response body", async () => {
  vi.stubGlobal(
    ...
    vi
      .fn()
      .mockResolvedValue(new Response("private response", { status: 403 })),
  );
  const report = vi.fn();
...
it("distinguishes oversized responses, timeouts and validation failures", async () => {
  ...
  expect(await fetchCashRate(report)).toBeNull();
  expect(report).toHaveBeenLastCalledWith(
    expect.stringContaining("validation: Missing or duplicate"),
  );
});
...
it("does not leak arbitrary network error text into the report", async () => {
  vi.stubGlobal(
    "fetch",
    vi.fn().mockRejectedValue(new Error("sensitive-provider-details")),
  );
  const report = vi.fn();
  expect(await fetchCashRate(report)).toBeNull();
...
it("does not substitute an older rate when the latest completed observation is missing", () => {
  expect(() =>
    parseCashRate(csv.replace("07-Sep-2026,4.35", "07-Sep-2026,"), now),
  ).toThrow("unavailable");
});
it("rejects the monthly series, changed identity, stale and future observations", () => {
  expect(() =>
    parseCashRate(csv.replace("FIRMMCRTD", "FIRMMCRT"), now),
  ).toThrow("series");
  expect(() =>
    parseCashRate(csv.replace("Cash Rate Target", "Interbank rate"), now),
  ).toThrow("Title");
  expect(() => parseCashRate(csv, new Date("2026-10-08"))).toThrow();
  expect(() =>
    parseCashRate(csv.replace("08-Sep-2026,,", "09-Sep-2026,4.35,"), now),
  ).toThrow("future");
});
