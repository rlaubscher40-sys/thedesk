...
describe("clusterByTitle", () => {
  it("preserves a short substantive update after original reporting has been read", () => {
    const articleText = Array.from({ length: 160 }, (_, i) => `evidenceword${i}`).join(" ");
    const first = {
    ...
    expect(clusterByTitle([first, { ...first, source: "Guardian" }])).toHaveLength(1);
    expect(
      clusterByTitle([
        first,
        {
          ...first,
          source: "Guardian",
          articleText: articleText + " The regulator subsequently withdrew the notice.",
        },
      ])
    ).toHaveLength(2);
  });
