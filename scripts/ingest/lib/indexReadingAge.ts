 ...
 * ABC topic indexes and SQM PDF lists mix current and months-old releases.
...
export function olderIndexPath(item: FetchedItem, now: Date): boolean {
  ...
  try {
    ...
    const sqm =
      host === "sqmresearch.com.au"
        ? /^\/uploads\/(\d{2})-(\d{2})-(\d{2})-[^/]+\.pdf$/i.exec(url.pathname)
        : null;
    const day = sqm
      ? `20${sqm[3]}-${sqm[2]}-${sqm[1]}`
      : host === "abc.net.au"
        ? /^\/news\/(20\d{2}-\d{2}-\d{2})\/[^/]+\/\d+$/.exec(url.pathname)?.[1]
        : null;
    if (!day) return false;
