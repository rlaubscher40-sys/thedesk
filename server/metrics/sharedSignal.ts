import { hasDailyObservations, historyChange } from "../../shared/metricPresentation";
...
export async function loadSharedSignal(
  metricKey: string,
  snapshotId?: string
  ...
  if (snapshotId !== undefined) {
    ...
    if (!snapshot || snapshot.metric.metricKey !== metricKey)
      throw new TRPCError({
        code: "NOT_FOUND",
        message: "That shared observation is unavailable. No newer value has been substituted.",
      });
    return hasDailyObservations(metricKey)
      ? snapshot
      : { ...snapshot, series: [], move: historyChange(snapshot.metric, []) };
  }
  ...
  return signalSnapshotSchema.parse({
    version: 1,
    metric,
    series: hasDailyObservations(metricKey) ? (histories[metricKey] ?? []) : [],
    move: null,
    deskTake: null,
    editionNumber: null,
  });
