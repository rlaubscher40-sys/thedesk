...
No edge may rely on a source older than 180 days or future-dated. A source with dateKind observation supplies an observation period, not a publication date. Never invent its release date. Raw dwelling approval counts alone cannot establish an investment advantage or an edge: they are not normalised for population or existing housing stock. Publication dates are not measurement periods; say if periods or definitions differ or are unknown and use unclear. National credit policy applies to both; it cannot prove a local edge. State-only data cannot prove a suburb or city claim. Distinguish listing volumes from housing stock and approvals from completions.
