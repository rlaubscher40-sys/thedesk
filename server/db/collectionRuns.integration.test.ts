...
beforeAll(async () => {
  ...
  if (!["127.0.0.1", "localhost"].includes(url.hostname) || url.pathname !== "/security_audit_test")
    throw new Error("Collection tests require the isolated local test database");
  ...
  await pool.query("CREATE TABLE IF NOT EXISTS collection_write_probe (value INT NOT NULL)");
  vi.doMock("./client", () => ({
    getDb: () => drizzle(pool, { logger: { logQuery: (query) => queryLog.push(query) } }),
  }));
...
it.skipIf(!testUrl)("uses only indexed reads when polling a completed collector", async () => {
  const lease = await runs.claimCollectionRun(key, day);
  expect(await runs.finishCollectionRun(lease!, "success")).toBe(true);
  queryLog.length = 0;
  for (let i = 0; i < 12; i++) expect(await runs.claimCollectionRun(key, day)).toBeNull();
  expect(queryLog).toHaveLength(12);
  for (const query of queryLog) {
    expect(query).toMatch(/^select /i);
    expect(query).toContain("`jobKey` = ?");
    expect(query).toContain("`runDate` = ?");
    expect(query).not.toMatch(/for update/i);
  }
});

it.skipIf(!testUrl)("preserves ordinary claim races and permanent publication locks", async () => {
  const ordinary = await import("./jobRuns");
  const jobKey = "efficiency-publication-probe";
  await pool.query("DELETE FROM job_runs WHERE jobKey = ? AND runDate = ?", [jobKey, day]);
  try {
    const claim = () => ordinary.claimJobRun(jobKey, day);
    expect((await Promise.all(Array.from({ length: 8 }, claim))).filter(Boolean)).toEqual([1]);
    // Old publication locks must not become stealable just because the pre-read exists.
    await pool.query(
      "UPDATE job_runs SET startedAt=DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 DAY) WHERE jobKey=? AND runDate=?",
      [jobKey, day]
    );
    expect(await claim()).toBe(0);
    await ordinary.markJobRun(jobKey, day, "failed");
    expect((await Promise.all(Array.from({ length: 8 }, claim))).filter(Boolean)).toEqual([2]);
    await ordinary.markJobRun(jobKey, day, "success");
    queryLog.length = 0;
    for (let i = 0; i < 12; i++) expect(await claim()).toBe(0);
    expect(queryLog).toHaveLength(12);
    expect(queryLog.every((query) => /^select /i.test(query))).toBe(true);
  } finally {
    await pool.query("DELETE FROM job_runs WHERE jobKey = ? AND runDate = ?", [jobKey, day]);
  }
});

it.skipIf(!testUrl)("allows one replica to claim and observes durable retry backoff", async () => {
  ...
  expect(
    await runs.claimCollectionRun(key, day, 3, new Date(later.getTime() + 3600_000))
  ).toBeNull();
});
...
it.skipIf(!testUrl)(
  ...
  async () => {
    const first = await runs.claimCollectionRun(key, day, 3, new Date(Date.now() - 16 * 60_000));
...
it.skipIf(!testUrl)("rolls back data written during a timed-out transaction", async () => {
  const lease = await runs.claimCollectionRun(key, day);
  let release!: () => void;
  let entered!: () => void;
...
it.skipIf(!testUrl)(
  ...
  async () => {
    ...
    expect(
      await runs.claimCollectionRun(key, day, 1, new Date(Date.now() + 16 * 60_000))
    ).toBeNull();
    ...
    await expect(runs.claimCollectionRun("instagram-daily", day)).rejects.toThrow(
