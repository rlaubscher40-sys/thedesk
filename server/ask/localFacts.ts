...
export async function retrieveLocalFacts(question: string): Promise<FactEvidence[]> {
  ...
  const labour = /\b(employment|unemployment|employed|participation|labour|labor|jobs)\b/i.test(
    question
  );
    ...
    work.push(
      localDatasets().then((datasets) => {
        const groups = matchLocalAreas(
          question,
          datasets.filter(
            (d) => broad || (d.sourceKey === "abs-sa2-population" ? population : rent)
          ),
          { question: true }
  ...
  if (
    approvals &&
    cities.length &&
    !/\b(suburb|postcode|lga|council|city of|western|eastern|northern|southern|north|south|east|west)\b/i.test(
      question
    )
  ) {
    work.push(
      getCityApprovals().then((data) => {
            ...
            return [
              {
                ...
                text:
                  city +
                  " (" +
                  approvalGeography(city) +
                  "): " +
                  read.total +
                  " dwellings approved, year to " +
                  rentPeriod(read.period) +
                  " (" +
                  read.period +
                  "). Measure: dwelling approvals; all dwelling types; original series; " +
                  (city === "Canberra" ? "territory" : "Greater Capital City Statistical Area") +
                  "; unit: dwellings. Sum of twelve consecutive observed months, not a single month's approvals, construction starts, completions, available homes or population-adjusted supply. " +
                  (read.preliminary ? "Includes preliminary observations. " : "") +
                  (read.revised ? "Includes revised observations. " : "") +
                  (period
                    ? "Requested historical window; not a claim about current conditions. "
                    : "") +
                  "Retrieved " +
                  data.retrievedAt +
                  "; retrieval is not publication. Larger counts do not establish an investment advantage.",
              },
            ];
          });
        });
        ...
        cities.flatMap((city) => {
          ...
          const rows = requestedPeriods.length
            ? data.observations
                ...
                .slice(0, 6)
            : latest && !rentIsOlder(latest, new Date().toISOString())
              ? [latest]
              : [];
          return rows.map((row) => ({
            cpiRent: { ...row },
      ...
      ? [
          ...new Set(
            requestedPeriods.map((period) =>
              period.length >= 7
                ? period.slice(0, 7)
                : currentWindow.from.startsWith(period)
                  ? currentWindow.from.slice(0, 7)
                  : ""
            )
          ),
        ]
          .map(planningPeriodWindow)
          .filter(
            (window): window is { from: string; to: string } =>
              Boolean(window) && window!.to <= currentWindow.to
          )
          .slice(0, 6)
      : [currentWindow];
    for (const window of windows)
      work.push(
        readPlanningSnapshots(NSW_PILOT_COUNCIL, window.from, window.to).then((rows) => {
          const row = rows[0];
          if (
            !row ||
            !row.completePagination ||
            row.from !== window.from ||
            row.to !== window.to ||
            row.councilName !== NSW_PILOT_COUNCIL ||
            (requestedPeriods.length &&
