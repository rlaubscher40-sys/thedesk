...
vi.mock("../core/publicRender", () => ({
  renderSignalCard: m.number,
  renderTrendCard: m.chart,
  renderDailyHookCoverCard: vi.fn(),
  renderIntelligenceCard: vi.fn(),
}));
vi.mock("node:fs", () => ({
  default: {
    existsSync: () => true,
...
>();
registerDistributionSeoRoutes({
  get: (path: string, ...rest: Array<(...args: any[]) => any>) => handlers.set(path, rest.at(-1)!),
...
beforeEach(() => {
  vi.clearAllMocks();
  m.read.mockResolvedValue(structuredClone(frozen));
  m.number.mockResolvedValue(Buffer.from("number"));
  m.chart.mockResolvedValue(Buffer.from("chart"));
});

async function request(route: string, query: Record<string, string>, accept = "text/html") {
  const res = {
  ...
  await handlers.get(route)!(
    { query, headers: { accept }, params: { metricKey: "cash_rate" } } as unknown as Request,
...
it("pins both canonical and image previews to the same saved dated observation", async () => {
  ...
  expect(m.histories).not.toHaveBeenCalled();
});

it.each(["/og/signals/:metricKey.png", "/og/charts/:metricKey.png"])(
  "renders %s from the frozen evidence",
  async (route) => {
...
it("serves the same specific sharing metadata to wildcard crawlers", async () => {
  const { res, next } = await request("/signals", { metric: "cash_rate", snapshot: id }, "*/*");
