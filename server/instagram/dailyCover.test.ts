...
describe("daily cover publication wiring", () => {
  it("never forwards cached factual inventions or a model rewrite to the automatic cover", async () => {
    ...
    expect(cover.story.title).toBe("Brisbane rents rose 5.3% in July 2026");
    expect(cover.body).not.toMatch(/guarantees|Perth|5\.3/);
  });
    ...
    async (variant) => {
      ...
      expect(m.render).toHaveBeenCalledWith(
        expect.objectContaining({
          kind: "cover",
          story: expect.objectContaining({ title: story.title }),
        }),
        0,
        4,
        variant
  ...
  it("shares the source-copy preview mapping without inventing or rewriting evidence", async () => {
    ...
    const [slide, index, count, variant] = m.render.mock.calls[0]!;
    expect(slide).toMatchObject({ kind: "cover", title: story.title });
    expect(slide).not.toHaveProperty("metrics");
    expect(slide).not.toHaveProperty("supporting");
    expect([index, count, variant]).toEqual([0, 4, "light"]);
  });
});
