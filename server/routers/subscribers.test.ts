...
vi.mock("../db", () => ({
  findSubscriberByEmail: mocks.find,
  createSubscriber: mocks.create,
  confirmSubscriber: mocks.confirm,
  listSubscribers: mocks.list,
  listFeedItems: mocks.feed,
  markDailyBriefSent: mocks.mark,
}));
vi.mock("../demo/store", () => ({ isDemoMode: mocks.demo }));
vi.mock("../core/mailer", () => ({
  sendConfirmEmail: mocks.mail,
  sendAlreadyConfirmedEmail: mocks.already,
  sendDailyBriefEmail: mocks.daily,
  editionUnsubscribeUrl: () => "https://thedesk.au/api/unsubscribe?sig=test",
}));
vi.mock("../core/publicLimits", () => ({
  reserveSubscriptionEmail: async () => true,
  releaseSubscriptionCooldown: async () => {},
}));
...
describe("subscription email acceptance", () => {
  ...
  it("waits for the provider before completing signup", async () => {
    ...
    let complete = false;
    const pending = caller.subscribe(input).then(() => {
      complete = true;
    });
  ...
  it("requires a new confirmation for a previously unsubscribed address", async () => {
    mocks.find.mockResolvedValue(
      subscriber({ confirmedAt: new Date(), unsubscribedAt: new Date() })
    );
  ...
  it("rejects honeypot submissions before email or database work", async () => {
    await expect(caller.subscribe({ ...input, _hp: "spam" })).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
...
describe("admin brief delivery status", () => {
  const admin = subscribersRouter.createCaller({
    req: {},
    res: {},
    user: { role: "admin" },
  } as TrpcContext);
  ...
  it("reports a provider rejection and leaves the send marker unchanged", async () => {
    ...
    await expect(admin.resendDailyBrief({ subscriberId: 1 })).rejects.toMatchObject({
      code: "INTERNAL_SERVER_ERROR",
    });
