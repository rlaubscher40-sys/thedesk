...
vi.mock("../db", () => ({
  listDailyMetrics: m.list,
  listMetricHistories: m.histories,
  listEditionSummaries: m.editions,
  storeSignalSnapshot: m.store,
  readSignalSnapshot: m.read,
}));
...
const metric = () => ({
  ...
  context: "Stored rate",
  previousValue: "4.1",
  asOf: new Date("2026-07-01Z"),
  updatedAt: new Date("2026-07-02Z"),
});
beforeEach(() => {
  vi.clearAllMocks();
  m.list.mockResolvedValue([metric()]);
  m.histories.mockResolvedValue({
    cash_rate: [
  ...
  m.number.mockResolvedValue(Buffer.from("image"));
  m.chart.mockResolvedValue(Buffer.from("chart"));
  m.quota.mockResolvedValue({ allowed: true });
  m.store.mockImplementation(async (input) => {
    saved = signalSnapshotSchema.parse(input);
    return id;
  });
...
it("freezes the Number's value, source, dates, history and attached edition", async () => {
  ...
  m.list.mockResolvedValue([
    { ...metric(), value: "5", source: "Changed", asOf: new Date("2026-08-01Z") },
  ]);
  ...
  const again = await signalsRouter
    .createCaller(ctx)
    .shareCard({ metricKey: "cash_rate", snapshot: id });
...
it("pins explicit percentage-point and interval copy on new Trends number shares", async () => {
  const card = await metricsRouter.createCaller(ctx).shareCard({ metricKey: "cash_rate" });
  expect(card.sharePath).toContain(`snapshot=${id}`);
  expect(saved.move).toBe("+0.25 percentage points · 1 June to 1 July");
...
it("rejects unknown or mismatched snapshots without loading or rendering current data", async () => {
  ...
  m.chart.mockClear();
  await expect(
    signalsRouter.createCaller(ctx).shareCard({ metricKey: "cash_rate", snapshot: "b".repeat(64) })
  ).rejects.toMatchObject({ code: "NOT_FOUND" });
  await expect(
    metricsRouter.createCaller(ctx).shareTrendCard({ metricKey: "other", snapshot: id })
  ).rejects.toMatchObject({ code: "NOT_FOUND" });
  expect(m.list).not.toHaveBeenCalled();
  expect(m.number).not.toHaveBeenCalled();
...
it("does not return a share when persistence fails", async () => {
  ...
  await expect(
    metricsRouter.createCaller(ctx).shareCard({ metricKey: "cash_rate" })
...
it("keeps render quotas before retrieval and rejects invalid snapshot IDs", async () => {
  ...
  await expect(
    signalsRouter.createCaller(ctx).shareCard({ metricKey: "cash_rate" })
  ).rejects.toMatchObject({ code: "TOO_MANY_REQUESTS" });
  expect(m.list).not.toHaveBeenCalled();
  await expect(
    metricsRouter.createCaller(ctx).shared({ snapshot: "invalid" })
