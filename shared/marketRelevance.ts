...
export const HOUSING_TOPIC_PATTERN =
  "(^|[^a-z])(housing|[0-9][0-9,]* (new |affordable |social )*homes|rents?|rental|dwellings?|mortgages?|rezoning|zoning|subdivid(e|ed|ing|ision|isions)|negative gearing|first.home buyers?|home loans?|residential (construction|supply|approvals|development)|building approvals|auction clearance|(house|home|property|apartment|unit|land) (prices?|values?|sales?|listings?|markets?|supply|affordability|investors?|investment|vacanc(y|ies)))([^a-z]|$)";
export function hasHousingEvidence(text: string): boolean {
  const subject = text
    .replace(/\s+/g, " ")
    .replace(
      /\bhousing\s+(?:(?:suspected|potential)\s+)?(?:nuclear|weapons?|missiles?|military|reactors?|servers?|equipment)\b/gi,
      "containing"
    );
