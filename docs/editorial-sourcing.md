...
Australian publication still requires a reviewed publisher domain and the existing eligibility threshold of 73, alongside geography, original publication date and readable-text checks. Eligibility and ranking are separate: a story that passes those checks may now have a displayed priority below 73. Search query names such as “Treasury” receive no source bonus.

For newly collected AU/Property stories, headline significance determines priority: reported policy decisions start at 90; market data developments and financial/housing disruption at 88; housing supply developments at 84; relevant reporting, analysis and proposals at 74; recognised industry/product partnerships at 66. Verified publisher domains add only 0–4 points. A primary publisher's generic interview cannot outrank a concrete housing development just through reputation. This is a deterministic editorial preference, not confidence, verification of a headline's claims, or a human-reviewed importance model. Ambiguous headlines default to relevant reporting; generic official releases can still qualify through their body but do not automatically receive a high ranking.

The same priority orders relevant local discovery candidates. Direct indexes receive a four-point discovery bonus, reduced from 40 so routine index entries cannot overwhelm stronger RSS candidates. The existing publisher reading opportunities and caps remain. After reading, equal-priority stories prefer newer original Sydney publication days instead of longer articles. Stories on the same original day retain their discovery order, avoiding invented times for day-only dates. Background text, generated takeaways and arbitrary numbers do not add ranking points.

The authenticated ingest boundary recomputes priority before insertion. Existing saved priorities and editor overrides are not rewritten. New priorities flow to feed ordering and the social selector's existing property-relevance tiers. This release does not trigger another one-time collection or alter the eligibility floor for international lanes. See the [ranking review](editorial-ranking-review-2026-09-10.md) for calibration and limits.
